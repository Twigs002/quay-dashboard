...
def get_current_week_bounds(now_sast):
      """Return (monday, friday) of the current Mon-Fri work week in SAST."""
      today = now_sast.date()
      # weekday(): Mon=0 ... Sun=6
...
def build_timespan(period_start, period_end, now_sast):
      """
          Build a DialFire timespan string 'N-Mday' where:
                N = days from today back to period_start (Monday)
                      M = days from today back to period_end (Friday)
                          """
      today = now_sast.date()
      days_to_start = (today - period_start).days
      days_to_end   = max(0, (today - period_end).days)
...
def fetch_json(url, params, label, tag, timeout=30, max_polls=8, headers=None):
      try:
                ...
                if r.status_code == 202:
                              poll_url = None
                              try:
                                                poll_url = r.json().get("url") or r.headers.get("Location")
      except Exception:
...
def fetch_lead_counts(cid, token, ts, label):
    ...
    # --- Approach 2: editsDef_v2 group0=user, group1=Lead_Status ---
    params2 = {
              "access_token": token,
              "asTree": "true",
              "timespan": ts,
...
def fetch_campaign(cid, token, index, total, period_start, period_end, ts):
    ...
    # Try the precise week timespan first, then fallbacks
      seen = set()
      unique_ts = []
...
def main():
    ...
    # Build DialFire timespan for Mon-Fri window
    ...
    # Legacy single-campaign env vars
