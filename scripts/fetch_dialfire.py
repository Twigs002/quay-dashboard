...
# -- Fetch lead/email counts using hs_lead_status as outer group --------------
def fetch_lead_counts(cid, token, ts, label):
      """
          Use editsDef_v2 with group0=hs_lead_status, group1=user, column0=completed.
              This gives us: for each status value -> list of users with call counts.
                  We map status -> seller/rental/email and accumulate per agent.

                      Falls back to group0=user, group1=hs_lead_status if the outer grouping
                          returns no nested data.
    ...
    # -- Approach 1: group0=hs_lead_status, group1=user -----------------------
    params = {
              "access_token": token,
              "asTree": "true",
              "timespan": ts,
    ...
    data = fetch_json(url, params, label, "leads: group0=hs_lead_status group1=user")
    if data is None:
              print(f"  [{label}] leads: 403 on lead-count approach 1")
        return result

    if isinstance(data, dict):
              groups = data.get("groups", [])
        print(f"  [{label}] leads: approach1 groups={type(groups).__name__}[{len(groups) if hasattr(groups,'__len__') else '?'}]")
        if isinstance(groups, list) and len(groups) > 0:
                      for status_item in groups:
                ...
                if status_val in SELLER_STATUSES:
                    ...
                    continue
                # Inner groups are users
                inner = status_item.get("groups", status_item.get("children", []))
                if not isinstance(inner, list):
                                      # No inner groups - try columns
                                      # (flat: status has columns = [count]) - skip, we need per-user
                                      continue
                for user_item in inner:
                    ...
                    if isinstance(cols, list) and len(cols) > 0:
                        ...
                        try:
                                                      count = int(round(float(list(cols.values())[0])))
except Exception:
                            count = 1
else:
                        count = 1
            ...
            if result:
                              print(f"  [{label}] leads: approach1 SUCCESS -> {result}")
                return result
            print(f"  [{label}] leads: approach1 got groups but no nested user data - trying approach 2")
    ...
    if data2 is None:
              print(f"  [{label}] leads: 403 on lead-count approach 2")
        return result

    if isinstance(data2, dict):
              groups2 = data2.get("groups", [])
        print(f"  [{label}] leads: approach2 groups={type(groups2).__name__}[{len(groups2) if hasattr(groups2,'__len__') else '?'}]")
        if isinstance(groups2, list) and len(groups2) > 0:
            ...
            if result:
                              print(f"  [{label}] leads: approach2 SUCCESS -> {result}")
                return result
            print(f"  [{label}] leads: approach2 got groups but no nested status data")
...
def parse_row(row):
    ...
    try:
              ...
              # Dialfire returns successRate as a fraction (0.0-1.0), convert to percent
              if 0.0 <= sr_float <= 1.0:
...
def main():
    ...
    # Replace entry for same week if exists, else prepend
