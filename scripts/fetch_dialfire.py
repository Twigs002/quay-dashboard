  ...
  GET /api/campaigns/{campaign_id}/reports/{template}/report/{locale}
    ?asTree=true  -> returns JSON
      ...
      JSON response structure:
        {
            "groups": {user_name: {"col0": val, "col1": val, ...}}   # may be [] if no data
                         OR {date: {"groups": {user: {...}}}}             # nested if group0=date
                             "groupDefs": [{"name": "user", ...}, ...]
                                   ...
                                   GitHub Secrets required:
                                     DIALFIRE_TENANT_ID     - tenant id (e.g. 3f88c548)
                                       DIALFIRE_TENANT_TOKEN  - tenant-level Bearer token (for campaign discovery)
                                         ...
                                         For each campaign (add as many as needed):
                                             ...
                                             CAMPAIGN_1_TOKEN       - campaign-level access token
                                               CAMPAIGN_2_ID          - campaign id  (e.g. N9EA67VHYX6HZHFG)
                                                 CAMPAIGN_2_TOKEN       - campaign-level access token
                                                   ...
                                                   If you only have a tenant token you can also set:
                                                     DIALFIRE_TENANT_TOKEN  and the script will discover campaigns automatically.
                                                     """
...
def get_campaigns_from_env():
      """
          Reads CAMPAIGN_1_ID / CAMPAIGN_1_TOKEN, CAMPAIGN_2_ID / CAMPAIGN_2_TOKEN, …
              Returns list of {"id": ..., "token": ...} dicts.
                  Falls back to tenant discovery if none found.
                      """
      campaigns = []
      i = 1
      while True:
...
def get_campaigns_from_tenant():
      """Discover campaigns via tenant API using TENANT_TOKEN."""
...
def fetch_json_report(url, params, label, tag, max_polls=5, poll_wait=4):
      """
          GET url with params + asTree=true.
              Handles HTTP 202 async polling (poll up to max_polls times).
                  Returns parsed JSON dict/list, [] on skip, None on auth error.
                      """
...
def extract_rows_from_tree(data, label, first_campaign=False):
      """
          Parse DialFire asTree=true JSON into a list of user stat dicts.

              Handles:
                    - groups as dict  {user: {col0: val, ...}}
                          - groups as list  [{user: ..., col0: val, ...}, ...]   (some API versions)
                                - groups as empty list [] -> returns []
    ...
    # col_map: {"completed": "col0", "success": "col1", ...}
    ...
    # ── Normalise groups_raw to a dict ────────────────────────────────────────
    if isinstance(groups_raw, list):
              if not groups_raw:
                            # Empty list -> no data for this campaign
                            return rows
                        # Some Dialfire versions return groups as a list of objects
                        # e.g. [{"user": "Alice", "col0": 12, "col1": 3}, ...]
              groups = {}
        for item in groups_raw:
                      if isinstance(item, dict):
                                        # Try to find the user/name key
    ...
    # ── Detect nesting (group0=date, group1=user) ─────────────────────────────
    ...
    if looks_like_dates and len(group_names) > 1:
        ...
        # Flat: {user: {col0: val, col1: val, ...}}
          for user_key, stats in groups.items():
            ...
            # declines = norespons + answeringmachines (for dialerStat)
...
# ── Parse a raw row dict into a dashboard agent dict ──────────────────────────
def parse_row(row):
...
def fetch_report(campaign, index, total):
    ...
    # ── Strategy 1: editsDef_v2 grouped by user (calls + success + workTime) ──
    ...
    # ── Strategy 2: dialerStat grouped by user (calls + connects + declines) ──
    ...
    # ── Strategy 3: editsDef_v2 grouped by date+user (nested) ────────────────
...
def main():
    ...
    if campaigns:
              print(f"Using {len(campaigns)} campaign(s) from environment secrets (CAMPAIGN_n_ID / CAMPAIGN_n_TOKEN)")
else:
        print("No CAMPAIGN_n_ID/TOKEN env vars found — falling back to tenant discovery...")
    ...
    # 2. Fetch stats for every campaign
    all_rows = []
    ...
    if os.path.exists(hist_path):
              try:
                            ...
                            # Only keep entries that are dicts with a weekStart key
