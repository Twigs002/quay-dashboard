...
def extract_rows(data, label):
    ...
    # Detect API format: list of {"value": name, "columns": [...]}
    # vs older dict format or list of named dicts
    def _parse_group_item(item, col_names_list):
              """Parse a single group item in either format and return a flat dict."""
              if not isinstance(item, dict):
                            return None
                        # New API format: {"value": "AgentName", "columns": [v0, v1, ...]}
                        if "columns" in item and isinstance(item["columns"], list):
                                      ...
                                      for i, cn in enumerate(col_names_list):
                                                        d[cn] = cols[i] if i < len(cols) else 0
                                                    return d
        # Old format: {"user": "AgentName", "col0": v0, "col1": v1, ...}
        # or {"name": "AgentName", "completed": v, ...}
        name = str(item.get("user", item.get("name", item.get("value", "")))).strip()
        if not name:
                      return None
        # Build dict using column names mapped from col0, col1...
        d = {"name": name}
        for i, cn in enumerate(col_names_list):
                      # Try direct name first, then col{i} key
                      if cn in item:
                                        d[cn] = item[cn]
elif f"col{i}" in item:
                d[cn] = item[f"col{i}"]
    ...
    # Check if this is a nested date->user structure
    date_re = re.compile(r"^\d{4}-\d{2}-\d{2}$")

    if isinstance(groups_raw, list):
              # Check if items are date-keyed (nested) or direct agent entries
              sample_values = []
        for item in groups_raw[:3]:
                      if isinstance(item, dict):
                                        v = str(item.get("value", item.get("name", item.get("user", ""))))
        ...
        for key, stats in groups_raw.items():
                      if key in ("total", "--", "") or not isinstance(stats, dict):
                                        continue
                                    # Check for nested date structure
                                    sample_keys = [k for k in list(stats.keys())[:3] if k not in ("total", "--", "")]
            if sample_keys and all(date_re.match(str(k)) for k in sample_keys) and len(grp_names) > 1:
                              # Nested date->user dict
                              inner_raw = stats.get("groups", {})
                for uk, ustats in (inner_raw.items() if isinstance(inner_raw, dict) else {}.items()):
                                      if uk in ("total", "--", "") or not isinstance(ustats, dict):
                                                                continue
                                                            parsed = _parse_group_item({**ustats, "name": uk}, col_names)
                    if parsed:
                                              rows.append(parsed)
else:
                parsed = _parse_group_item({**stats, "name": key}, col_names)
...
def parse_row(row):
    ...
    # workTime can be in seconds (large int) or hours (small float)
    work_raw = _safe_float(work_val)
    # Heuristic: if value > 24 it's likely seconds, otherwise hours
    work_hrs = work_raw / 3600.0 if work_raw > 24 else work_raw
    ...
    if sr is None or sr == "":
        ...
        sr = _safe_float(sr)
        # successRate might be stored as a fraction (0-1) rather than percent
        if 0.0 < sr <= 1.0 and success > 0 and calls > 0:
                      computed = success / calls
            if abs(sr - computed) < 0.01:
                              sr = round(sr * 100, 1)
